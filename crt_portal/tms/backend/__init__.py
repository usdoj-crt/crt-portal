...
class TMSEmailBackend(BaseEmailBackend):
    ...
    def __init__(self, api_root=None, api_auth_token=None, fail_silently=False, **kwargs):
    ...
    def open(self):
        ...
        if self.connection:
            return False
        try:
            self.connection = TMSClient(api_root=self.api_root, api_auth_token=self.api_auth_token)
        except Exception:
