...
(function(root, dom) {
  function init() {
    ...
    if (relatedReportButton !== null) {
      relatedReportButton.addEventListener('click', (e) => { sendGAClickEvent('view related reports') });
    }
    const navItems = document.getElementsByClassName("usa-nav__primary-item");
    for (var i = 0; i < navItems.length; i++) {
     const navItem = navItems[i];
      navItem.addEventListener('click', (e) => sendGAPublicClickEvent('main nav ' + e.target.innerText))
     }
