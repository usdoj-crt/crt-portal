...
(function (root, dom) {
  ...
  remove_buttons.forEach(function (btn) {
    btn.onclick = function (event) {
      ...
      var attachment_filename = this.getAttribute('data-attachment-filename'); // modal elements
      ...
      var yes_button = document.getElementById('attachment-removal--yes'); // add the to-be-removed filename to the modal

      filename_el.innerText = attachment_filename; // user clicks no on removal confirmation

      no_button.onclick = function (event) {
        event.preventDefault(); // do nothing, just hide the modal
      ...
      yes_button.onclick = function (event) {
        event.preventDefault(); // get the form for removing this particular attachment

        var form_id = "complaint-view-remove-attachment-".concat(attachment_id);
        var form = document.getElementById(form_id); // submit the removal form
...
(function () {
  function showForm() {
    ...
    showFollowUpQuestions(primaryComplaint.value); // reveal Buttons
  ...
  function addFormUpdateEvents(form) {
    // Show follow-up fields if any
    var primaryComplaint = document.getElementById('id_primary_complaint');
    showFollowUpQuestions(primaryComplaint.value); // Generate listenable events for all form inputs, using `change` for IE11 support
  ...
  function showFollowUpQuestions(selectedReason) {
    var allOptionalFields = ['public_or_private_employer', 'employer_size', 'public_or_private_school', 'inside_correctional_facility', 'correctional_facility_type', 'commercial_or_public_place', 'other_commercial_or_public_place'];
    var followupMapping = {
  ...
  var primaryIssues = document.getElementById('id_primary_complaint'); // add listeners
...
(function (root) {
  var DEPT_ADDRESS = {
    deptOfEd: '<p id="form-letterhead--dept-addressee">U.S. Department of Education<br>Office for Civil Rights<br>Lyndon Baines Johnson Department of Education Bldg.<br>400 Maryland Avenue, SW<br>Washington, DC 20202-1100<br></p>',
  ...
  var addReferralAddress = function addReferralAddress(option) {
    var addressee = document.getElementById('form-letterhead--addressee');
    var deptAddressee = document.getElementById('form-letterhead--dept-addressee');
    ...
    var letterName = option.innerText.replace(/\(.+\)$/, '').trim();
  ...
  var reportId = document.getElementById('template-report-id').value;
  options.addEventListener('change', function (event) {
    event.preventDefault();
    var index = event.target.selectedIndex;
    var option = event.target.options[index];
    var value = event.target.value;
    window.fetch('/api/responses/' + value + '/?report_id=' + reportId).then(function (response) {
  ...
  var setLanguageCookie = function setLanguageCookie(lang) {
    document.cookie = 'form-letter-language' + '=' + lang;
  };

  var getLanguageCookie = function getLanguageCookie() {
  ...
  var applyTemplateLanguageFilter = function applyTemplateLanguageFilter() {
    var language_select = document.getElementById('template-language-select');
    var selected_language = language_select.value; // form letters for languages other than the one selected

    var toHide = document.querySelectorAll("#intake_select > option.usa-select:not([data-language=".concat(selected_language, "])")); // form letters for the language that is selected

    var toShow = document.querySelectorAll("#intake_select > option.usa-select[data-language=".concat(selected_language, "]"));

    var _iterator = _createForOfIteratorHelper(toHide),
        _step;
    ...
    var _iterator2 = _createForOfIteratorHelper(toShow),
        _step2;
  ...
  var copyContents = function copyContents(event) {
    // Text-only letter
    if (!letter.hidden) {
      ...
      document.execCommand('copy');
      document.body.removeChild(el); // HTML content
  ...
  var printContents = function printContents(event) {
    var letterhead = document.getElementById('form-letterhead');
    var letter_placeholder = document.getElementById('form-letter--placeholder'); // Text-only letter

    if (!letter.hidden) {
      var el = document.createElement('p');
      el.append(letter.value);
      letter_placeholder.classList.add('form-letter-text');
      letter_placeholder.appendChild(el); // HTML letter
    } else if (!letter_html.hidden) {
      var _el = letter_html.cloneNode(true); // Prevent id collision


      _el.id = _el.id + '_rand' + Math.floor(Math.random() * 1000000);
      letter_placeholder.appendChild(_el);
    }
  ...
  function B(e, t) {
    var u = e.replace(/\|/g, function (e, t, u) {
      ...
      return n ? "|" : " |";
        ...
        n = 0;
    if (u[0].trim() || u.shift(), 0 < u.length && !u[u.length - 1].trim() && u.pop(), u.length > t) u.splice(t);else for (; u.length < t;) {
      u.push("");
    }
    ...
    return t.space = function (e) {
      ...
      if (e) {
        ...
        if (t.header.length === t.align.length) {
          ...
          for (i = t.header.length, u = 0; u < i; u++) {
            t.header[u].tokens = [], this.lexer.inlineTokens(t.header[u].text, t.header[u].tokens);
          }

          for (i = t.rows.length, u = 0; u < i; u++) {
            for (r = t.rows[u], n = 0; n < r.length; n++) {
              r[n].tokens = [], this.lexer.inlineTokens(r[n].text, r[n].tokens);
            }
          }
      ...
      if (t) {
        ...
        if (!this.options.pedantic && /^</.test(e)) {
          ...
          -1 < n && (r = (0 === t[0].indexOf("!") ? 5 : 4) + t[1].length + n, t[2] = t[2].substring(0, n), t[0] = t[0].substring(0, r).trim(), t[3] = "");
        }
      ...
      if (e) {
        var u,
            t = "@" === e[2] ? "mailto:" + (u = D(this.options.mangle ? t(e[1]) : e[1])) : u = D(e[1]);
        return {
          type: "link",
          raw: e[0],
          text: u,
  ...
  var I = function () {
    ...
    return n.lex = function (e) {
      ...
      if (this.tokens.links) {
        ...
        if (0 < D.length) for (; null != (n = this.tokenizer.rules.inline.reflinkSearch.exec(o));) {
          D.includes(n[0].slice(n[0].lastIndexOf("[") + 1, -1)) && (o = o.slice(0, n.index) + "[" + y("a", n[0].length - 2) + "]" + o.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex));
        }
      }

      for (; null != (n = this.tokenizer.rules.inline.blockSkip.exec(o));) {
        o = o.slice(0, n.index) + "[" + y("a", n[0].length - 2) + "]" + o.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
      }
      ...
      for (; r;) {
        if (l || (a = ""), l = !1, !(this.options.extensions && this.options.extensions.inline && this.options.extensions.inline.some(function (e) {
        ...
        }))) if (u = this.tokenizer.escape(r)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.tag(r)) r = r.substring(u.raw.length), (e = t[t.length - 1]) && "text" === u.type && "text" === e.type ? (e.raw += u.raw, e.text += u.text) : t.push(u);else if (u = this.tokenizer.link(r)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.reflink(r, this.tokens.links)) r = r.substring(u.raw.length), (e = t[t.length - 1]) && "text" === u.type && "text" === e.type ? (e.raw += u.raw, e.text += u.text) : t.push(u);else if (u = this.tokenizer.emStrong(r, o, a)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.codespan(r)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.br(r)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.del(r)) r = r.substring(u.raw.length), t.push(u);else if (u = this.tokenizer.autolink(r, R)) r = r.substring(u.raw.length), t.push(u);else if (this.state.inLink || !(u = this.tokenizer.url(r, R))) {
          ...
          }(), u = this.tokenizer.inlineText(i, T)) r = r.substring(u.raw.length), "_" !== u.raw.slice(-1) && (a = u.raw.slice(-1)), l = !0, (e = t[t.length - 1]) && "text" === e.type ? (e.raw += u.raw, e.text += u.text) : t.push(u);else if (r) {
        ...
        if (o = e[k], !(this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[o.type]) || !1 === (A = this.options.extensions.renderers[o.type].call({
          ...
          case "list":
            for (D = o.ordered, E = o.start, c = o.loose, r = o.items.length, a = "", u = 0; u < r; u++) {
              f = (p = o.items[u]).checked, g = p.task, h = "", p.task && (F = this.renderer.checkbox(f), c ? 0 < p.tokens.length && "paragraph" === p.tokens[0].type ? (p.tokens[0].text = F + " " + p.tokens[0].text, p.tokens[0].tokens && 0 < p.tokens[0].tokens.length && "text" === p.tokens[0].tokens[0].type && (p.tokens[0].tokens[0].text = F + " " + p.tokens[0].tokens[0].text)) : p.tokens.unshift({
                type: "text",
                text: F
...
// EXTERNAL MODULE: ./crt_portal/static/vendor/marked.min.js
var marked_min = __webpack_require__(760);
// EXTERNAL MODULE: ./crt_portal/static/js/components/edit_contact_info.js
var edit_contact_info = __webpack_require__(826);
// EXTERNAL MODULE: ./crt_portal/static/js/components/edit_details.js
var edit_details = __webpack_require__(427);
// EXTERNAL MODULE: ./crt_portal/static/js/components/modal.js
var modal = __webpack_require__(671);
// EXTERNAL MODULE: ./crt_portal/static/js/components/form_letter.js
var form_letter = __webpack_require__(536);
...
// EXTERNAL MODULE: ./crt_portal/static/js/components/routing_guide.js
var routing_guide = __webpack_require__(429);
// EXTERNAL MODULE: ./crt_portal/static/js/components/attachments.js
var attachments = __webpack_require__(892);
;// CONCATENATED MODULE: ./crt_portal/static/js/show.js
